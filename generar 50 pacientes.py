...
def generar_ids_equipo_sin_repetir(cantidad):
    ids_generados = set()  # Conjunto para mantener valores únicos
    ids_equipos = []
    while len(ids_equipos) < cantidad:
        ...
        if id_equipo not in ids_generados:
            ids_generados.add(id_equipo)
            ids_equipos.append(id_equipo)
    return ids_equipos
...
def simular_y_guardar_archivos():
    ...
    numeros_pacientes = generar_numeros_aleatorios_sin_repetir(50)
    ids_equipos = generar_ids_equipo_sin_repetir(50)
